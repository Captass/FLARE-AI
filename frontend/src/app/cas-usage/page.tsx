...
export default function CasUsagePage() {
  return (
    <PublicPageShell
      ...
      description="Automatisation business. Preuve active aujourd'hui: chatbot Facebook assiste pour convertir plus vite."
      statusBlocks={[
        {
    ...
    >
      <section className="rounded-[30px] border border-black/12 bg-white p-6 shadow-[0_20px_65px_rgba(15,23,42,0.08)] md:p-8">
        <h2 className="text-3xl font-black tracking-tight text-black md:text-4xl">Avant / Apres par secteur</h2>
        <p className="mt-3 max-w-3xl text-sm leading-relaxed text-black/80 md:text-base">
          Lire vite. Comprendre vite. Voir le gain direct.
        </p>
        ...
        <div className="mt-6 space-y-4">
          {USE_CASES.map((item) => (
            <article
              key={item.sector}
              className="rounded-2xl border border-black/10 bg-[#fbf7f0] p-5 shadow-[0_10px_30px_rgba(15,23,42,0.06)]"
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="text-xl font-black tracking-tight text-black">{item.sector}</h3>
                <span className="inline-flex items-center gap-2 rounded-full border border-orange-500/35 bg-[#fff2e3] px-3 py-1 text-xs font-black uppercase tracking-[0.08em] text-black">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-black/10 bg-white p-3">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-black/70">Avant</p>
                  <p className="mt-2 text-sm font-medium leading-relaxed text-black/85">{item.before}</p>
                </div>
                <div className="rounded-xl border border-orange-500/30 bg-[#fff2e3] p-3">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-black/70">Apres automation</p>
                  <p className="mt-2 text-sm font-medium leading-relaxed text-black/90">{item.after}</p>
                </div>
              </div>
              <p className="mt-3 text-sm font-semibold text-black">{item.highlight}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          return (
            <article
              key={card.title}
              className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,0.05)]"
            >
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-black/10 bg-[#f8f2e7]">
                <Icon className="h-5 w-5 text-black" />
              </div>
              <h3 className="mt-3 text-xl font-black tracking-tight text-black">{card.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-black/80">{card.copy}</p>
            </article>
          );
        })}
      </section>

      <section className="rounded-[26px] border border-black/10 bg-white p-6 md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-black/70">Prochaine etape</p>
            <h2 className="mt-1 text-3xl font-black tracking-tight text-black">Passez sur une offre adaptee</h2>
            <p className="mt-2 text-sm text-black/80">Choisissez votre rythme. Activez votre premier flux.</p>
          </div>
          <Link
            href="/offres"
